...
export async function executeJSInDockerContainer(
  ...
  */
  ...
  try {
    output = await sh(
      // `docker exec ${containerName} sh -c "deno ${dockerFilePath} >> ${dockerLogFilePath}"`
      // this cmd force deno to check the types and then executes the script
      `docker exec ${containerName} sh -c "deno check ${dockerFilePath} && deno run ${dockerFilePath} >> ${dockerLogFilePath}"`,
    )
  } catch (err) {
